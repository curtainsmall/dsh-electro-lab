...
export function restartRequired(home: string): boolean {
  try {
    const state = JSON.parse(readFileSync(statePath(home), "utf8")) as Record<
      string,
      unknown
    >;
...
export function upsertExternalTool(
  home: string,
  ...
  switch (s.type) {
    case ExternalParamType.Quantity:
      if (s.kind === undefined || !QUANTITY_KIND_NAMES.includes(s.kind)) {
        errors.push(
          `${path}: quantity type requires a known kind (lowercase QuantityKind names)`,
        );
      }
      break;
    case ExternalParamType.String:
      if (
        ...
        (!Array.isArray(s.enum) ||
          s.enum.some((item) => typeof item !== "string"))
      ) {
    ...
    case ExternalParamType.Array:
      if (s.items === undefined)
        errors.push(`${path}: array type requires an items declaration`);
      else validateParamSpec(s.items, `${path}.items`, errors);
      break;
    default:
      errors.push(
        `${path}: unknown type "${String(s.type)}" (one of ${Object.values(ExternalParamType).join(", ")})`,
      );
  }
}
