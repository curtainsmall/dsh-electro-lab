...
/** 外部 fn 的接线块：语境见它即自动包 http/file 执行器作 run。 */
...
export class FnRegistry {
  ...
  register(fn: FnDef): void {
    if (!NAME_PATTERN.test(fn.id)) throw new ToolError(`fn id "${fn.id}" must match ^[a-z][a-z0-9_]{0,63}$`, ToolErrorCode.ContextArgs)
  ...
  require(id: string): FnDef {
    ...
    if (fn === undefined) throw new ToolError(`unknown fn "${id}"`, ToolErrorCode.ContextUnknownFn)
    return fn
  }
