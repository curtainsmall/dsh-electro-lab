...
function startGenerateJob(ctx: Context, record: Record, directory: string, fileName: string, language: ArticleLanguage, format: ArticleFormat, compile: boolean): string {
  ...
  void (async () => {
    try {
      ...
      // PDF compilation exists for LaTeX only (xelatex); Markdown articles are
      // single .md files. A compile failure never fails the job — it is
      // reported as compileError on the done snapshot.
      if (compile && isLatex) {
