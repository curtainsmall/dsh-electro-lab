...
describe('resonance', () => {
  it('computes f0, Q, and bandwidth (textbook check)', () => {
    ...
    const { resonantFrequency, qualityFactor, bandwidth } = resonance(1e-3, 1e-9, 10, CircuitMode.Series)
  ...
  it('computes parallel Q', () => {
    const { qualityFactor } = resonance(1e-3, 1e-9, 1000, CircuitMode.Parallel)
...
describe('RC transient', () => {
  it('charges to 63.2% after one time constant (textbook check)', () => {
    ...
    const { voltage, current, timeConstant } = rcTransient(SwitchingMode.Charge, 5, 0, 1e3, 1e-6, 1e-3)
  ...
  it('discharges exponentially', () => {
    const { voltage } = rcTransient(SwitchingMode.Discharge, 0, 10, 1e3, 1e-6, 1e-3)
...
describe('RL transient', () => {
  it('charges toward Vs/R with time constant L/R', () => {
    ...
    const { current, timeConstant } = rlTransient(SwitchingMode.Charge, 10, 0, 100, 1e-3, 1e-5)
