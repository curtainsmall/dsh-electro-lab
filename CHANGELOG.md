...
- Client **Records** panel in SSH style: a lightning nav entry in the sidebar foot (beside Settings) toggles a panel that takes over the center column — back-to-session title bar, Config/Records tabs, content below — with **one page listing every settled electro-lab run across all sessions**. The host traces sessions without touching them: a pure fold over `tool/call` / `tool/result` events groups runs (closed on turn end, a user message, a foreign tool in between, or an idle gap) and persists each settled run — raw question inputs (follow-ups accumulate), the five-step assistant texts, the exact tool outputs (`tool/result` content) and tool usage — to a plugin-owned store on disk (`~/.dsh-electro-lab/records.json`, deliberately outside `$DSH_HOME` so the records survive session deletion, restarts and even a full DSH uninstall; override with `DSH_ELECTRO_LAB_HOME`). A host listener summarizes the question inputs into one self-contained question through the session's own model route and attaches it to the stored record. The client reads the store through a same-origin endpoint (`/api/dsh-electro-lab/records`, polled every 5 s) — no session-log writes, no custom event types (the session persistence read path refuses unknown out-of-repo events). The Config tab is an empty placeholder for now.
