...
- **Record protocol**: `record_question` opens a record and submits the question text (it does not depend on tool results, so it comes before any calculation), `record_analyse` submits the analysis, `record_answer` submits the answer text and settles the record immediately. Only what happens between the markers is recorded; the marker calls themselves never appear in `calls`. Disorder semantics produce **error records** (`error: { type, message }` on an otherwise ordinary record, data preserved): a second `record_question` settles the open record as `duplicate-start` and opens a fresh one; `record_answer` with no open record keeps an empty `duplicate-end` record; a record settled with no tool call is `incomplete`. When the model merges the whole five-part template into the answer text, the manager splits it out and fills only the gaps (direct markers win, then collected open texts, then the split template parts; the answer keeps its segment with the tables dropped). The markers are read from the `tool/call` events like every other tool — the event keeps the raw arguments JSON as a string (parsing happens in the execution channel), so the manager parses it itself; one event channel, no bridge. Event shapes follow the real dsh-session vocabulary (`assistant/message` text in `data.message.content`; the `tool/result` pairing key in the ToolResultBlock). The plugin mounts once — the preset no longer carries the plugin row, so nothing is registered twice. The RecordManager owns ALL record state — the JSONL archive (a settled record is appended the moment it settles) and the interrupted-open snapshot (`open-record.json`, same directory, persisted after every event and restored by the constructor after a restart); the session log is never re-read.
