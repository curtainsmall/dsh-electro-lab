...
- Client **Records** panel in SSH style: a lightning nav entry in the sidebar rail — DOM-mounted beside the SSH, task-board and skills entries, Font Awesome-style icon rendered like the shell's 16px line icons — toggles a panel that takes over the center column — back-to-session title bar, Config/Records tabs, content below — with **one page listing every settled electro-lab run across all sessions**. The host traces sessions without touching them: a pure fold over `tool/call` / `tool/result` events groups runs (closed on turn end, a user message, a foreign tool in between, or an idle gap) and appends each settled run as **one immutable JSONL line** to a plugin-owned store on disk (`~/.dsh-electro-lab/records.jsonl`, deliberately outside `$DSH_HOME` so the records survive session deletion, restarts and even a full DSH uninstall; override with `DSH_ELECTRO_LAB_HOME`). Records are built from sessions but never feed back into them: nothing downstream rebuilds sessions from this file, and the records page reads records only. Each run is recorded **one-shot** — the answer template opens with a **Question** part that restates the complete question (follow-ups merged into one full question), so the record's first analyse text IS the consolidated question; no separate summarizer or question line exists. The record schema mirrors the template: `question` / `analyse` / `answer` are single paragraphs, while `calls` and `results` are structured object arrays keeping every detail (raw arguments JSON, full output text, error identity). The five-step template is restructured accordingly (question, analysis with formulas — analyse/plan merged —, tool calls, results, answer) in the template skill and the packaged preset persona. The client reads the store through a same-origin endpoint (`/api/dsh-electro-lab/records`, polled every 5 s) — no session-log writes, no custom event types (the session persistence read path refuses unknown out-of-repo events). The Config tab is an empty placeholder for now.

### Changed

- **Bracket protocol**: `record_start` / `record_end` marker tools (no-op calls, native `tool/call` events — safe carriers that survive session replay) bracket one five-step calculation, and only what happens between the brackets is recorded — texts before `record_start` and after `record_end` are never captured. `record_start` carries the consolidated question and the host hashes it (sha256) — exactly one `record_start` per answer — a second one settles the open record as a duplicate-start error record and opens a new one: there is no fold, no question key, nothing to judge. Disorder semantics produce **error records** (`error: { type, message }` on an otherwise ordinary record, data preserved): a duplicate start settles the open record as `duplicate-start` and opens a fresh one; a duplicate end keeps an empty `duplicate-end` record; a record settled with no tool call at all is `incomplete` (the events were not enough for a calculation). `record_end` settles immediately and carries the final texts as payload arguments (`question` / `analyse` / `answer`): tool-phase models write their texts after the last tool call, so the model copies them into the closing marker; a missing payload field falls back to the event-stream texts. The markers are read from the `tool/call` events like every other tool — the event keeps the raw arguments JSON as a string (parsing happens in the execution channel), so the manager parses the payload itself; one event channel, no bridge. The session trace and HTTP endpoint are process singletons, so mounting the plugin both globally and through a preset can no longer settle every record twice. The marker calls themselves never appear in `calls`. The RecordManager now owns ALL record state — the disk archive (a settled record is appended the moment it settles) and the interrupted-open snapshot (`open-record.json`, same directory, persisted after every event and restored by the constructor after a restart). Keeping the state on disk beats replaying the session log, which is never re-read (no fold). All record functionality lives in one module — `src/records.ts` (the separate record-store / open-record modules are gone): the manager takes the two file paths and owns the open record, the JSONL archive and the snapshot file, with archive reads through the exported `readRecordArchive`.
