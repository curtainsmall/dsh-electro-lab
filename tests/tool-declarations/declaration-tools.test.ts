...
describe('external_fns_add', () => {
  it('stores a new declaration and reports the pending restart', async () => {
    const result = await run('external_fns_add', { declaration: TOOL }, home)
  ...
  it('fails when the name already exists', async () => {
    await run('external_fns_add', { declaration: TOOL }, home)
    await runFailure('external_fns_add', { declaration: { ...TOOL, description: 'again' } }, home, /external_fns_update/)
    expect(readDeclarations(home)).toHaveLength(1)
  })

  it('fails on an invalid declaration without touching the archive', async () => {
    await runFailure('external_fns_add', { declaration: { ...TOOL, parameters: { x: { type: 'float' } } } }, home, /parameter "x": unknown type/)
  ...
  it('accepts a declaration without the enabled flag', async () => {
    ...
    const result = await run('external_fns_add', { declaration: noFlag }, home)
    expect(result).toEqual({ name: 'sample_echo', changed: 'added', restartRequired: true })
  })
})

describe('external_fns_update', () => {
  ...
  it('fails when the name is not declared yet', async () => {
    await runFailure('external_fns_update', { declaration: TOOL }, home, /external_fns_add/)
  })

  it('fails on an invalid declaration', async () => {
    await run('external_fns_add', { declaration: TOOL }, home)
    await runFailure('external_fns_update', { declaration: { ...TOOL, enabled: 'yes' } }, home, /enabled must be a boolean/)
  })
})

describe('external_fns_delete', () => {
  it('removes a declared tool and reports the pending restart', async () => {
    await run('external_fns_add', { declaration: TOOL }, home)
    const result = await run('external_fns_delete', { name: 'sample_echo' }, home)
    expect(result).toEqual({ name: 'sample_echo', changed: 'deleted', restartRequired: true })
    expect(readDeclarations(home)).toEqual([])
  })

  it('fails when the name is not declared', async () => {
    await runFailure('external_fns_delete', { name: 'ghost' }, home, /ghost/)
    expect(readDeclarations(home)).toEqual([])
  })
})
