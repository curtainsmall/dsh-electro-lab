...
description: "Electro-lab record protocol: record_question opens a record and submits the question, record_analyse submits the analysis, record_answer submits the answer and settles — the structured five-step content lives in the record, so the chat answer stays natural (no template headings in the session)"
...
The structured five-step content (question, analysis, tool calls, results, answer) is captured by the RECORD, not by the session chat. Answer the user naturally; the record markers carry the structure.
...
- The chat answer is natural language: no template tables or numbered part headings in the session — the record is the structured presentation.
